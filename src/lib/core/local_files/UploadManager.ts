...
class FileUploadManager {
  ...
  constructor(files: FileArray, allowedMimeTypes: string[], allowedExtensions: string[], filePath: string) {
    this.files = files;
  ...
  public removeFiles(): void {
    this.uploadedFiles.forEach((item) => {
      FileManager.instance.removeFileFromServer(FileManager.instance.joinPath([this.filePath, item.name]));
      // FileManager.instance.removeFileFromServer(FileManager.instance.joinPath([item.tempFilePath, item.name]));
    });

    // this.uploadedFiles = [];
    // this.singleFileName = undefined;
  }

  private setFileName(fileName: string, regenerate: Boolean = true) {
    const _fileName = slugify(fileName, { lower: true });
    this.singleFileName = regenerate ? Date.now().toString() + _fileName : _fileName;
  }
